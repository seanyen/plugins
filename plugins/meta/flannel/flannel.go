// Copied from https://github.com/flannel-io/cni-plugin/blob/main/flannel.go
...
func Main() {
	fullVer := fmt.Sprintf("CNI %s plugin %s (%s/%s) commit %s built on %s", Program, Version, runtime.GOOS, runtime.GOARCH, Commit, buildDate)
