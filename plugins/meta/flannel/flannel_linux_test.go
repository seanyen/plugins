...
FLANNEL_IPMASQ=true
`
	writeSubnetEnv := func(contents string) string {
	...
	makeInputIPAM := func(ipamType, routes, extra string) string {
		c := "{\n"
		if len(ipamType) > 0 {
	...
	makeInput := func(inputIPAM string, subnetFile string) string {
		ipamPart := ""
		if len(inputIPAM) > 0 {
			ipamPart = ",\n  \"ipam\":\n" + inputIPAM
		}
